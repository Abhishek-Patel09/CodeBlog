...
class SnippetForm(forms.ModelForm):
    ...
    class Meta:
        model = Snippet
        fields = ('title', 'original_code', 'language', 'exposure', )
        widgets = {
            'title': forms.TextInput(attrs={'class': 'selectpicker form-control', 'placeholder': 'Enter Title (optional)'}),
            'original_code' : forms.Textarea(attrs={'class': 'form-control', 'rows':'10', 'spellcheck': 'false'}),
            'language' : forms.Select(attrs={'class': 'selectpicker foo form-control', 'data-live-search': 'true', 'data-size': '5'}),
            'exposure': forms.Select(attrs={'class': 'selectpicker from-control'}),
        }
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            self.fields['exposure'].choices = Preference.exposure_choices
            self.initial = request.user.profile.get_preferences()
        else:
            self.fields['exposure'].choices = \
                [ (k, v) for k, v in Preference.exposure_choices if k != 'private']

            l = Language.objects.get(name='Plain Text')
            self.initial = {'language': l.id, 'exposure': 'public'}
...
class CreateUserForm(UserCreationForm):
    ...
    def save(self, request):
        ...
        #subject = render_to_string('codeblog/email/activation_subject.txt', context)   #In atom new line gets added automatically and subject header gives error for that , that's why hard-coded
        email = render_to_string('codeblog/email/activation_email.txt', context)

        send_mail('Action Required to Complete the Account Creation - CodeBlog', email, settings.DEFAULT_FROM_EMAIL, [user.email])
...
class SettingForm(forms.ModelForm):
    ...
    class Meta:
        model = Author
        fields = ('default_language', 'default_exposure', 'private')
        widgets = {
